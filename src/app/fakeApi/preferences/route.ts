...
function buildResponsePayload(data: typeof DEFAULT_PREFERENCES) {
  return {
    data,
    error_code: 'S000000',
    error_msg: 'No Error',
...
export async function GET(req: NextRequest) {
  const preferences = getDemoData(req, 'demo_preferences', DEFAULT_PREFERENCES)

  const response = NextResponse.json(buildResponsePayload(preferences))
  // Only if the cookie does not exist yet (first visit)
  if (!req.cookies.get('demo_preferences')) {
    setDemoData(response, 'demo_preferences', preferences, req)
  }
  return response
}
...
export async function PATCH(req: NextRequest) {
  ...
  const preferences = getDemoData(req, 'demo_preferences', DEFAULT_PREFERENCES)
  const mutablePreferences = preferences as Record<
    string,
