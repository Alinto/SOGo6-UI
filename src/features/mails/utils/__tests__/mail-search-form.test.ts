...
describe('buildMailSearchParams — include_subfolders', () => {
  it('sends include_subfolders: false when unset (simple bar default) since the API defaults it to true', () => {
    const params = buildMailSearchParams({
      ...defaultSearchFormValues,
      subject: 'invoice',
    })
    expect(params.include_subfolders).toBe(false)
  })
...
describe('mailSearchParamsToQueryText', () => {
  ...
  it('keeps match:any when OR is set even with only one criterion, so the chosen operator survives a round-trip', () => {
    const params: MailSearchParams = { subject: 'invoice', operator: 'OR' }
    expect(mailSearchParamsToQueryText(params)).toBe(
      'match:any subject:invoice'
    )
  })
  ...
  it('quotes single-word free text that looks like a key:value token', () => {
    // Otherwise "to:5" would render unquoted and be silently reinterpreted
    // as the `to:` operator the next time the query bar is parsed.
    const params: MailSearchParams = { text: 'to:5' }
    expect(mailSearchParamsToQueryText(params)).toBe('"to:5"')
  })
...
describe('mailSearchParamsToUrlSearchParams', () => {
  ...
  it('omits fields that are unset/false/empty', () => {
    const params: MailSearchParams = { subject: 'invoice', is_flagged: false }
    expect(mailSearchParamsToUrlSearchParams(params).toString()).toBe(
      'subject=invoice'
    )
  })

  it('serializes attachment/date/read/flag/subfolders/labels/operator', () => {
    const params: MailSearchParams = {
      has_attachment: true,
      attachment_type: ['pdf', 'png'],
      date_range: { start: '2024-01-01', end: '2024-02-01' },
    ...
    const urlParams = mailSearchParamsToUrlSearchParams(params)
    expect(urlParams.get('has_attachment')).toBe('1')
    expect(urlParams.get('attachment_type')).toBe('pdf,png')
    expect(urlParams.get('date_start')).toBe('2024-01-01')
    expect(urlParams.get('date_end')).toBe('2024-02-01')
    expect(urlParams.get('is_read')).toBe('read')
    expect(urlParams.get('is_flagged')).toBe('1')
    expect(urlParams.get('include_subfolders')).toBe('1')
    expect(urlParams.get('labels')).toBe('Work,Perso')
    expect(urlParams.get('operator')).toBe('OR')
  })

  it('serializes is_read: false as "unread"', () => {
    expect(
      mailSearchParamsToUrlSearchParams({ is_read: false }).get('is_read')
    ).toBe('unread')
  })

  it('omits the folder param when scoped to "all", includes it otherwise', () => {
    expect(
      mailSearchParamsToUrlSearchParams({ folders: ['all'] }).has('folder')
    ).toBe(false)
    expect(
      mailSearchParamsToUrlSearchParams({ folders: ['INBOX/Work'] }).get(
        'folder'
      )
    ).toBe('INBOX/Work')
  })
...
describe('urlSearchParamsToMailSearchParams', () => {
  it('is the reverse of mailSearchParamsToUrlSearchParams for a full set of params', () => {
    const params: MailSearchParams = {
      text: 'quarterly',
      subject: 'invoice',
      from: 'jane',
    ...
    const urlParams = mailSearchParamsToUrlSearchParams(params)
    expect(urlSearchParamsToMailSearchParams(urlParams)).toEqual(params)
  })

  it('defaults folders to ["all"] when no folder param is present', () => {
    expect(
      urlSearchParamsToMailSearchParams(new URLSearchParams()).folders
    ).toEqual(['all'])
  })

  it('ignores unrecognized/malformed values', () => {
    const urlParams = new URLSearchParams('is_read=maybe&operator=XOR')
    const params = urlSearchParamsToMailSearchParams(urlParams)
    expect(params.is_read).toBeUndefined()
    expect(params.operator).toBeUndefined()
  })
})

describe('buildAdvancedSearchPath', () => {
  it('builds a path with no query string for empty params', () => {
    expect(buildAdvancedSearchPath('0', {})).toBe(
      `/u/0/${ADVANCED_SEARCH_ROUTE_SEGMENT}`
    )
  })
