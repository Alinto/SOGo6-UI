...
export function MailsSearch() {
  ...
  // A new search's result set has its own page count, unrelated to whatever
  // page the previous view was on — keep the URL's `page` param from
  // stranding the user on an empty/truncated page (same convention as
  // ListFilter's filter changes).
  ...
  // The top bar switches from the simple (field-scope-driven) input to an
  // advanced-style "key:value" query bar whenever the active search can't be
  ...
  // Re-synced on every open (see `closeAdvancedModal` resetting the ref on
  // close) rather than once per search-activation lifecycle — otherwise
  ...
  const inboxPath = flatFolders.find(
    (f) => normalizeFolderType(f.type) === 'INBOX'
  )?.path
  const draftsPath = flatFolders.find(
    (f) => normalizeFolderType(f.type) === 'DRAFT'
  )?.path
  const sentPath = flatFolders.find(
    (f) => normalizeFolderType(f.type) === 'SENT'
  )?.path
  ...
  const activeTargets = useMemo(
    () =>
      Array.from(
        new Set(fieldScope.map((scope) => SEARCH_FIELD_SCOPE_TARGET[scope]))
      ),
    [fieldScope]
  )
  const readField =
  ...
  const handleSimpleSubmit = (values: SearchFormValues) => {
    ...
    const isOnAdvancedSearchRoute =
      currentFolderPath === ADVANCED_SEARCH_ROUTE_SEGMENT
    // The advanced-search route is a pseudo-folder, not a real one to search
    // in — this is reachable when a simple-bar-compatible advanced search is
    // reloaded/opened fresh (see isSimpleBarCompatible), which prefills the
    // bar in simple mode while still parked on that route. Keep whatever
    // real folder scope the active search already had instead of sending
    // the pseudo-segment itself as a folder.
    const folderScope = isOnAdvancedSearchRoute
      ? (mailSearch.params?.folders?.[0] ?? 'all')
      : currentFolderPath || 'all'
    ...
    if (isOnAdvancedSearchRoute) {
      push(buildAdvancedSearchPath(accountId, params))
    } else {
      resetPageParam()
    }
  }
  ...
  const submitAdvancedQueryText = () => {
    const baseFolder =
      mailSearch.params?.folders?.[0] ?? (currentFolderPath || 'all')
  ...
  // Resets the prefill ref on close so the modal re-syncs from the active
  // search's current params the next time it's opened, instead of showing
  // whatever was there the last time it was primed.
  const closeAdvancedModal = (open: boolean) => {
    setAdvancedOpen(open)
    if (!open) advancedPrefilledRef.current = false
  }

  // The advanced modal's search is bookmarkable/shareable: submitting it
  // navigates to a dedicated `/advanced-search?...` URL (rather than staying
  // on the current folder, like the simple bar does) so the search survives
  ...
  const clearSearch = () => {
    ...
    setBarMode('simple')
    setAdvancedQueryText('')
    // Clearing from the advanced-search route has nowhere else to fall back
    // to (it's not a real folder) — return to the inbox, like other exits
    // from a virtual/removed location do (see folder-messages-error-fallback).
    if (currentFolderPath === ADVANCED_SEARCH_ROUTE_SEGMENT) {
      push(`/u/${accountId}/INBOX`)
    }
  }
      ...
      <Form {...simpleForm}>
        ...
        <div className="text-foreground flex items-center gap-2">
          ...
          <Button
            ...
            size="icon"
            className="shrink-0"
            onClick={() => closeAdvancedModal(true)}
...
export default memo(MailsSearch)
