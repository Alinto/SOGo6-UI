...
import { schema } from './calendar-categories-schema'
...
interface Props {
  ...
  update: (data: UserCalendarCategory) => void
}

const LabelsForm: React.FC<Props> = ({ data, update }) => {
  const t = useTranslations('US_CALENDARS')

  const fetchedData = data ? mapApiToCalendarCategorySettings(data) : undefined
          ...
          {fields.map((label, i) => (
            <div key={label.id} className="flex items-center gap-4">
              ...
              <Button onClick={() => remove(i)} size="icon" variant={'ghost'}>
                <AccessibleIcon
                  label={t('categories.accessibility.icon.delete.string', {
                    name: label.name,
                  })}
                >
