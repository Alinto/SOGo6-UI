...
describe('calendarsMessagesT', () => {
  describe('key resolution', () => {
    it('resolves a top-level CALENDARS key', () => {
      expect(calendarsMessagesT('loading')).toBe('Loading...')
    })

    it('resolves a nested dotted key', () => {
      expect(calendarsMessagesT('eventForm.attendees.no_data')).toBe(
  ...
  describe('interpolation', () => {
    it('replaces placeholders with provided values', () => {
      expect(
        calendarsMessagesT('eventForm.attendees.timeline_hour', { hour: 14 })
      ).toBe('14h')
    })

    it('returns the raw message when no values are passed', () => {
      expect(calendarsMessagesT('eventForm.attendees.timeline_hour')).toBe(
        '{hour}h'
      )
    })
    ...
    it('replaces multiple placeholders in one message', () => {
      expect(
        calendarsMessagesT('eventForm.attendees.tooltip_slot_free', {
          name: 'Alice',
          time: '10:00',
        })
