...
- `LOGIN_PREFILL_EMAIL` / `LOGIN_PREFILL_PASSWORD` - Optional; runtime login prefill served via `GET /env` (readable by anyone who can hit `/env`). In production images also set `LOGIN_PREFILL_ENABLED=true` (staging/QA only). Legacy `NEXT_PUBLIC_LOGIN_PREFILL_*` on the server is still merged in `/env` if `LOGIN_*` are unset.
