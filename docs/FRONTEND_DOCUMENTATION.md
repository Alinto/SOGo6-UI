...
**RSVP / attendance.** Accept/Tentative/Decline buttons when the user is an attendee (`visualization/index.tsx`, `mail-invitation-widget.tsx`) → `usePostEventAttendanceMutation` (`calendars-api.ts:553-608`). The backend sends an iMIP REPLY to the organizer via `_send_imip` (`InterfaceApiCalendarCalendar.py`) when attendance is updated.
...
| `POST /events/<key>/attendance` | `postEventAttendance` | ✅ | Sends iMIP REPLY to organizer (backend) |
...
- **iMIP attendance**: FE sends PARTSTAT via `postEventAttendance` (calendar view and mail invitation widget). The backend updates the local event and sends an iMIP REPLY to the organizer via SMTP (`InterfaceApiCalendarCalendar._send_imip`).
